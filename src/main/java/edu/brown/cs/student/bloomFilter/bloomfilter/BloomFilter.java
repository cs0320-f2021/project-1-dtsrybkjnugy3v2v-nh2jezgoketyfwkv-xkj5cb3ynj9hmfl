package edu.brown.cs.student.bloomFilter.bloomfilter;
...
public class BloomFilter<E> implements Serializable {
  ...
  // number of elements actually added to the Bloom filter so far
  private int numberOfAddedElements;
  // number of hash functions
  private int k;
  // encoding used for storing hash values as strings
  static final Charset CHARSET = StandardCharsets.UTF_8;
  // hash function to use (MD5, SHA1, etc.)
  static final String HASH_NAME = "SHA1";
   ...
   * the total size of the Bloom filter and the number of expected elements.
   *
   * @param bitSetSize              defines how many bits should be used in total for the filter.
   * @param expectedNumberOElements defines the maximum number of elements the filter is expected
   *                                to contain.
   */
   ...
   * @param cs  specifies the encoding of the input data.
   * @return digest as long.
   */
  public static int createHash(String val, Charset cs) {
    return createHash(val.getBytes(cs));
  }
   ...
   * int's are produced. For each call to digest a salt is prepended to the data.
   ...
   */
  public boolean mightContainAll(Collection<? extends E> c) {
    for (E element : c) {
  ...
  /**
   * Get expected number of bits per element when the Bloom filter is full. This value is set by
   * the constructor when the Bloom filter is created. See also getBitsPerElement().
   *
